#include "..\\stdafx.h"
#include "..\\include\\TextBox\\TextBox.h"
#include "..\\include\\Base\\CMathLib.h"
using namespace OwLib;

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void TextBoxA::CursorDown()
{
...
void TextBoxA::InsertWord( const String& str )
{
	String text = GetText();
	int textLength = (int)text.length();
...
bool TextBoxA::IsLineVisible( int index, double visiblePercent )
{
	int rangeSize = (int)m_ranges.size();
	...
	return CMathLib::IsLineVisible(indexTop, indexBottom, cell, floor, m_lineHeight, visiblePercent);
}

void TextBoxA::SetMovingIndex( int sIndex, int eIndex )
{
...
TextBoxA::TextBoxA()
{
	...
	m_textAlign = HorizontalAlignA_Left;
	m_textChanged = false;
	m_wordWrap = false;
	TICK = 500;
	m_tick = 500;
...
int TextBoxA::GetLinesCount()
{
	return (int)m_lines.size();
}

int TextBoxA::GetLineHeight()
{
	return m_lineHeight;
}

void TextBoxA::SetLineHeight( int lineHeight )
{
	m_lineHeight = lineHeight;
}

vector<WordLine> TextBoxA::GetLines()
{
	return m_lines;
}

bool TextBoxA::IsMultiline()
{
	return m_multiline;
}

void TextBoxA::SetMultiline( bool multiline )
{
...
wchar_t TextBoxA::GetPasswordChar()
{
	return m_passwordChar;
}

void TextBoxA::SetPasswordChar( wchar_t passwordChar )
{
	m_passwordChar = passwordChar;
	m_textChanged = true;
}

bool TextBoxA::IsReadOnly()
{
	return m_readOnly;
}

void TextBoxA::SetReadOnly( bool readOnly )
{
	m_readOnly = readOnly;
}

bool TextBoxA::IsRightToLeft()
{
	return m_rightToLeft;
}

void TextBoxA::SetRightToLeft( bool rightToLeft )
{
	m_rightToLeft = rightToLeft;
	m_textChanged = true;
}

_int64 TextBoxA::GetSelectionBackColor()
{
	return m_selectionBackColor;
}

void TextBoxA::SetSelectionBackColor( _int64 selectionBackColor )
{
	m_selectionBackColor = selectionBackColor;
}

_int64 TextBoxA::GetSelectionForeColor()
{
	return m_selectionForeColor;
}

void TextBoxA::SetSelectionForeColor( _int64 selectionForeColor )
{
	m_selectionForeColor = selectionForeColor;
}

int TextBoxA::GetSelectionLength()
{
	return m_selectionLength;
}

void TextBoxA::SetSelectionLength( int selectionLength )
{
	m_selectionLength = selectionLength;
}

int TextBoxA::GetSelectionStart()
{
	return m_selectionStart;
}

void TextBoxA::SetSelectionStart( int selectionStart )
{
...
String TextBoxA::GetTempText()
{
	return m_tempText;
}

void TextBoxA::SetTempText( const String& tempText )
{
	m_tempText = tempText;
}

_int64 TextBoxA::GetTempTextForeColor()
{
	return m_tempTextForeColor;
}

void TextBoxA::SetTempTextForeColor( _int64 tempTextForeColor )
{
	m_tempTextForeColor = tempTextForeColor;
}

HorizontalAlignA TextBoxA::GetTextAlign()
{
	return m_textAlign;
}

void TextBoxA::SetTextAlign( HorizontalAlignA textAlign )
{
	m_textAlign = textAlign;
}

bool TextBoxA::IsWordWrap()
{
	return m_wordWrap;
}

void TextBoxA::SetWordWrap( bool wordWrap )
{
...
bool TextBoxA::CanRedo()
{
	return (int)m_redoStack.size() > 0;
}

bool TextBoxA::CanUndo()
{
	return (int)m_undoStack.size() > 0;
}

void TextBoxA::ClearRedoUndo()
{
	while (!m_undoStack.empty())
		m_undoStack.pop();
...
int TextBoxA::GetContentHeight()
{
	int contentHeight = DivA::GetContentHeight();
	int bottom = 0;
	int count = (int)m_ranges.size();
...
int TextBoxA::GetContentWidth()
{
	int wmax = DivA::GetContentWidth();
	int cwidth = 0;
	int rangeSize = (int)m_ranges.size();
...
String TextBoxA::GetControlType()
{
	return L"TextBox";
}

void TextBoxA::GetProperty( const String& name, String *value, String *type )
{
	if (name == L"lineheight")
	{
		*type = L"int";
		*value = CStr::ConvertIntToStr(GetLineHeight());
	}
	else if (name == L"multiline")
	{
		*type = L"bool";
		*value = CStr::ConvertBoolToStr(IsMultiline());
	}
	...
	{
		*type = L"bool";
		*value = CStr::ConvertBoolToStr(IsReadOnly());
	}
	else if (name == L"righttoleft")
	{
		*type = L"bool";
		*value = CStr::ConvertBoolToStr(IsRightToLeft());
	}
	else if (name == L"selectionbackcolor")
	{
		*type = L"color";
		*value = CStr::ConvertColorToStr(GetSelectionBackColor());
	}
	else if (name == L"selectionforecolor")
	{
		*type = L"color";
		*value = CStr::ConvertColorToStr(GetSelectionForeColor());
	}
	...
	{
		*type = L"color";
		*value = CStr::ConvertColorToStr(GetTempTextForeColor());
	}
	else if (name == L"textalign")
	{
		*type = L"enum:HorizontalAlignA";
		*value = CStr::ConvertHorizontalAlignToStr(GetTextAlign());
	}
	else if (name == L"wordwrap")
	{
		*type = L"bool";
		*value = CStr::ConvertBoolToStr(IsWordWrap());
	}
	else
	{
...
vector<String> TextBoxA::GetPropertyNames()
{
...
String TextBoxA::GetSelectionText()
{
	String text = GetText();
...
void TextBoxA::OnChar( wchar_t ch )
{
	if (!m_readOnly)
	{
		DivA::OnChar(ch);
...
void TextBoxA::OnCopy()
{
	...
	{
		string str;
		CStr::wstringTostring(str, selectionText);
...
void TextBoxA::OnKeyDown( char key )
{
	m_isKeyDown = true;
	if (GetNative()->GetHost()->IsKeyPress(0x11))
	{
...
void TextBoxA::OnKeyUp( char key )
{
	ControlA::OnKeyUp(key);
...
void TextBoxA::OnLostFocus()
{
	ControlA::OnLostFocus();
	StopTimer(m_timerID);
	m_isKeyDown = false;
	m_showCursor = false;
	m_selectionLength = 0;
...
void TextBoxA::OnMouseDown( const POINT& mp, MouseButtonsA button, int clicks, int delta )
{
...
void TextBoxA::OnMouseMove( const POINT& mp, MouseButtonsA button, int clicks, int delta )
{
...
void TextBoxA::OnMouseUp( const POINT& mp, MouseButtonsA button, int clicks, int delta )
{
	m_isMouseDown = false;
	DivA::OnMouseUp(mp, button, clicks, delta);
}

void TextBoxA::OnPaintForeground( CPaint *paint, const RECT& clipRect )
{
    ...
    if ((width > 0) && (height > 0))
    {
        ...
        if (!m_multiline)
        {
            ...
            scrollH -= offsetX;
            if (m_textAlign == HorizontalAlignA_Center)
            {
                offsetX = -((width - padding.right) - ((int) (lastRange.right - firstRange.left))) / 2;
            }
            else if (m_textAlign == HorizontalAlignA_Right)
            {
...
void TextBoxA::OnPaste()
{
	if (!m_readOnly)
	{
		string insert = GetNative()->GetHost()->Paste();
		String wstr;
		CStr::stringTowstring(wstr, insert);
...
void TextBoxA::OnSizeChanged()
{
	ControlA::OnSizeChanged();
	if (m_wordWrap)
	{
		m_textChanged = true;
		Invalidate();
...
void TextBoxA::OnTextChanged()
{
	m_textChanged = true;
	ControlA::OnTextChanged();
}

void TextBoxA::OnTimer( int timerID )
{
...
void TextBoxA::SelectAll()
{
	m_selectionStart = 0;
	m_selectionLength = (int)GetText().length();
}

void TextBoxA::SetProperty( const String& name, const String& value )
{
	if (name == L"lineheight")
	{
		SetLineHeight(CStr::ConvertStrToInt(value));
	}
	else if (name == L"multiline")
	{
		SetMultiline(CStr::ConvertStrToBool(value));
	}
	else if (name == L"passwordchar")
	{
	...
	{
		SetReadOnly(CStr::ConvertStrToBool(value));
	}
	else if (name == L"righttoleft")
	{
		SetRightToLeft(CStr::ConvertStrToBool(value));
	}
	else if (name == L"selectionbackcolor")
	{
		SetSelectionBackColor(CStr::ConvertStrToColor(value));
	}
	else if (name == L"selectionforecolor")
	{
		SetSelectionForeColor(CStr::ConvertStrToColor(value));
	}
	else if (name == L"temptext")
	{
		SetTempText(value);
	...
	{
		SetTempTextForeColor(CStr::ConvertStrToColor(value));
	}
	else if (name == L"textalign")
	{
		SetTextAlign(CStr::ConvertStrToHorizontalAlign(value));
	}
	else if (name == L"wordwrap")
	{
		SetWordWrap(CStr::ConvertStrToBool(value));
	}
	else
	{
