...
void CMathLib::M001( int index, int n, double s, double m, double high, double low, 
...
double CMathLib::M002( double value, double *listForAvedev, int listForAvedev_length, double avg )
{
	int i = 0;
...
double CMathLib::M003( int index, int n, double value, struct LPDATA last_MA )
{
	double sum = 0.0;
	if (last_MA.mode == 0)
	{
...
double CMathLib::M004( int index, int n, double value, struct LPDATA last_SUM )
{
	double sum = 0.0;
	if (last_SUM.mode == 0)
	{
...
double CMathLib::M005( int n, int weight, double value, double lastWMA )
{
	return (value * weight + (n - weight) * lastWMA) / n;
}

double CMathLib::M006( int n, double value, double lastEMA )
{
	return (value * 2.0 + lastEMA * (n - 1)) / (n + 1);
}

double CMathLib::M007( double *list, int length, double avg, double standardDeviation )
{
	int i = 0;
	if (length > 0)
	{
...
double CMathLib::GetMin( double *list, int length )
{
	double min = 0.0;
	int i = 0;
	for (i = 0; i < length; i++) {
...
double CMathLib::GetMax( double *list, int length )
{
	double max = 0.0;
	int i = 0;
	for (i = 0; i < length; i++) {
...
double CMathLib::M010( double *list, int length )
{
	int i = 0;
	double sum = 0.0;
	if (length > 0)
...
double CMathLib::M011( double *list, int length )
{
	double sum = 0.0;
	int i = 0;
	for (i = 0; i < length; i++) {
...
int CMathLib::M012( double min, double max, int yLen, int maxSpan, int minSpan, int defCount, double *step, int *digit )
{
	double sub = max - min;
...
void CMathLib::M013( int index, double close, double p, double *sxp, int *sxi, double *exp, int *exi, int *state, int *cStart, int *cEnd, double *k, double *b )
{
	bool reverse = false;
	bool ex = false;
	if (index == 0)
...
void CMathLib::M014( double *list, int length, float *k, float *b )
{
	int i = 0;
	double sumX = 0.0;
	double sumY = 0.0;
...
double CMathLib::M015( double close, double lastSma, int n, int m )
{
	return (close * m + lastSma * (n - m)) / n;
}

void CMathLib::M105( int x1, int y1, int x2, int y2, int *x, int *y, int *w, int *h )
{
	*x = x1 < x2 ? x1 : x2;
	*y = y1 < y2 ? y1 : y2;
	*w = abs(x1 - x2);
...
double CMathLib::M106( float x1, float y1, float x2, float y2, float oX, float oY )
{
	if (x1 - oX != x2 - oX) {
...
void CMathLib::M107( float x1, float y1, float x2, float y2, float oX, float oY, float *k, float *b )
{
	*k = 0.0;
	*b = 0.0;
	if (x1 - oX != x2 - oX)
...
void CMathLib::M108( float width, float height, float *a, float *b )
{
	*a = width / 2.0;
	*b= height / 2.0;
}

bool CMathLib::M109( float x, float y, float oX, float oY, float a, float b )
{
	x -= oX;
	y -= oY;
...
void CMathLib::M110( float x1, float y1, float x2, float y2, float x3, float y3, float *oX, float *oY, float *r )
{
...
void CMathLib::M124( float x1, float y1, float x2, float y2, float x3, float y3, float *x4, float *y4 )
{
	*x4 = x1 + x3 - x2;
	*y4 = y1 + y3 - y2;
}

double CMathLib::M129( int tm_year, int tm_mon, int tm_mday, int tm_hour, int tm_min, int tm_sec, int tm_msec )
{
	struct tm t;
	time_t tn;
	t.tm_sec = tm_sec;
...
void CMathLib::M130( double num, int *tm_year, int *tm_mon, int *tm_mday, int *tm_hour, int *tm_min, int *tm_sec, int *tm_msec )
{
...
void CMathLib::M132( RECT *bounds, SIZE *parentSize, SIZE *oldSize, bool anchorLeft, bool anchorTop, bool anchorRight, bool anchorBottom )
{
...
void CMathLib::M133( RECT *bounds, RECT *spaceRect, SIZE *cSize, int dock )
{
	...
	else if (dock == DockStyleA_Fill)
	{
		bounds = spaceRect;
	}
	else if (dock == DockStyleA_Left)
	{
		bounds->left = spaceRect->left;
...
void CMathLib::M135( int layoutStyle, RECT *bounds, PADDING *padding, PADDING *margin, int left, int top, int width, int height, int tw, int th, POINT *headerLocation )
{

}

void CMathLib::M137( int resizePoint, int *left, int *top, int *right, int *bottom, POINT *nowPoint, POINT *startMousePoint )
{
	switch (resizePoint)
	{
	case 0: 
...
bool CMathLib::IsLineVisible( int indexTop, int indexBottom, int cell, int floor, int lineHeight, double visiblePercent )
{
	if (indexTop < cell)
	{
		indexTop = cell;
