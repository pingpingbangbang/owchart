#include "..\\stdafx.h"
#include "..\\include\\Layout\\GroupBox.h"
...
String GroupBoxA::GetControlType()
{
	return L"GroupBox";
}

void GroupBoxA::OnPaintBorder( CPaint *paint, const RECT& clipRect )
{
	FONT *font = GetFont();
	int width = GetWidth();
	...
	ControlA::CallPaintEvents(EVENTID::PAINTBORDER, paint, clipRect);
}

void GroupBoxA::OnPaintForeground( CPaint *paint, const RECT& clipRect )
{
	String text = GetText();
	if ((int)text.length() > 0)
	{
